...
tissue_list = names(smr_list0)[grepl('Brain',names(smr_list0))]
tissue_list = unique(unlist(str_split(tissue_list,'\\.',simplify = T)[,1]))
tissue_list = setdiff(tissue_list,'BrainMeta')
...
df %>% 
  ...
  ggplot(aes(x=reorder_within(Var2, Freq,Var1),
             y=Freq))+
  # geom_point(aes(color=Var3))+
  geom_bar(aes(fill=cohorts),stat="identity")+
  ...
  theme_bw()+
  coord_flip()+
  facet_wrap(cohorts~Var3,scales = 'free')+
...
df %>% 
  mutate(Var3 = factor(Var3,levels=c('IBD','CD','UC'))) %>% 
  ggplot(aes(x=reorder_within(Var2, ws,Var1),
             y=ws))+
  # geom_point(aes(color=Var3))+
  geom_bar(aes(fill=cohorts),stat="identity")+
  # scale_color_gradientn(colours = rev(mapal))+
...
df %>% 
  mutate(Var3 = factor(Var3,levels=c('IBD','CD','UC'))) %>% 
  ggplot(aes(x=reorder_within(Var2, ws2,Var1),
             y=ws2))+
  # geom_point(aes(color=Var3))+
  geom_bar(aes(fill=cohorts),stat="identity")+
  # scale_color_gradientn(colours = rev(mapal))+
